...
export default function UploadZone({
  ...
  const handleFileChange = (selectedFile: File) => {
    ...
    if (selectedFile.size > 14 * 1024 * 1024) {
      // 14MB max limit for robust payloads
      setErrorMessage(
        "File exceeds 14MB. Please upload a smaller study guide PDF.",
      );
      return;
    }
  ...
  const handleGenerateClick = (e: React.FormEvent) => {
    ...
    if (!file && !text.trim()) {
      setErrorMessage(
        "Please select a training PDF file, paste study notes or click on one of our premium quick presets below.",
      );
      return;
    }
    onGenerate({
  ...
  return (
    <div className="max-w-4xl mx-auto py-8 px-4 sm:px-6 animate-fade-in">
      ...
      <div className="text-center mb-10">
        <div className="inline-flex items-center space-x-2 bg-slate-50 border border-slate-100 px-3 py-1 rounded-full text-[11px] font-mono font-medium text-slate-600 mb-4 transition-all">
        ...
        <p className="text-slate-500 font-sans text-sm sm:text-base max-w-xl mx-auto leading-relaxed">
          Upload any lecture syllabus, textbook chapter, or paste rough research
          notes to craft high-fidelity, interactive training materials in
          seconds.
        </p>
      </div>
      ...
      {!userEmail && (
        <div className="mb-8 p-6 rounded-3xl bg-slate-950 text-white border border-slate-900 shadow-xl flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden transition-all hover:border-slate-800">
          <div className="absolute -right-16 -top-16 w-40 h-40 rounded-full bg-slate-800/20 blur-2xl pointer-events-none" />
          ...
          <div className="text-left space-y-1.5 z-10 max-w-lg">
            ...
            <p className="text-[11.5px] text-slate-400 font-sans leading-relaxed">
              ...
              quiz score metrics, and connect with your absolute student
              credentials.
            </p>
          </div>
      ...
      <form
        ...
        className="bg-white border border-slate-100 rounded-3xl p-6 sm:p-8 shadow-sm transition-all duration-300 hover:shadow-md"
      >
        {/* Toggle Mode Control */}
        <div className="mb-8">
          ...
          <div className="grid grid-cols-2 gap-2 bg-slate-50 p-1.5 rounded-2xl border border-slate-100 max-w-md mx-auto relative">
            ...
            >
              <FileCode2
                className={`h-4 w-4 ${mode === "flashcards" ? "text-white" : "text-slate-400"}`}
              />
            ...
            >
              <HelpCircle
                className={`h-4 w-4 ${mode === "quiz" ? "text-white" : "text-slate-400"}`}
              />
              <div className="text-left">
                <p className="font-semibold block leading-none">
                  Evaluation Quiz
                </p>
                <span className="block text-[10px] opacity-70 mt-0.5">
                  Assessment testing
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          ...
          <div className="flex flex-col">
            ...
            <div
              className={`border-2 border-dashed rounded-2xl p-5 flex flex-col items-center justify-center cursor-pointer transition-all duration-300 min-h-47.5 text-center ${
                isDragActive
            ...
            >
              {file ? (
                <div className="flex flex-col items-center space-y-3">
                  ...
                  <div>
                    <h4
                      className="text-xs font-semibold text-slate-950 truncate max-w-50"
                      title={file.name}
                    >
                      {file.name}
                    </h4>
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    <p className="text-xs text-slate-900 font-medium font-sans">
                      Drag & drop your study PDF here, or{" "}
                      <span className="text-indigo-600 hover:text-indigo-800 font-semibold underline decoration-2 underline-offset-2">
                        browse
                      </span>
                    </p>
          ...
          <div className="flex flex-col">
            <label className=" text-xs font-mono uppercase tracking-widest text-slate-700 font-semibold mb-2 flex items-center space-x-1.5">
              <SlidersHorizontal className="h-3.5 w-3.5 text-slate-600" />
              <span>Or Paste Raw Text Notes</span>
            </label>
            <textarea
              className="resize-none flex-1 border border-slate-200 rounded-2xl p-4 text-xs font-sans text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-1 focus:ring-slate-900 focus:border-slate-900 min-h-47.5 bg-white transition-all focus:bg-slate-50/30"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          >
            ...
            <p className="text-[10px] text-slate-500 font-sans mt-1 line-clamp-2">
              Explore superposition, quantum tunneling, and entanglement
              dynamics.
            </p>
          </button>
          ...
          >
            ...
            <p className="text-[10px] text-slate-500 font-sans mt-1 line-clamp-2">
              Volcanic concrete chemistry, siphoning aqueducts and gravitational
              structural shapes.
            </p>
          </button>
          ...
          >
            ...
            <p className="text-[10px] text-slate-500 font-sans mt-1 line-clamp-2">
              Discover chemotrophic structures, bioluminescence reactions, and
              gigantism phenomena.
            </p>
          </button>
        </div>
