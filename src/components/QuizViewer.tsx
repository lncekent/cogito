      ...
      {!quizFinished ? (
        ...
        <div>
          ...
          <div className="grid grid-cols-1 gap-3.5 mb-6">
            {activeQuestion.options.map((option, idx) => {
              ...
              if (isAnsweredThis) {
                if (isCorrectOption) {
                  optionStyle =
                    "border-emerald-500 bg-emerald-500/10 text-emerald-950 font-semibold";
                  badgeStatus = (
                  ...
                  optionStyle =
                    "border-slate-100 opacity-60 bg-slate-50/50 text-slate-500 pointer-events-none";
                }
              }
              ...
              return (
                ...
                >
                  <div className="flex items-center space-x-3 text-left">
                    <span
                      className={`h-6 w-6 rounded-lg font-mono flex items-center justify-center border text-[11px] ${
          ...
          <div className="flex justify-end pt-3">
            ...
            >
              <span>
                {currentIdx < quiz.length - 1 ? "NEXT QUESTION" : "FINISH EXAM"}
              </span>
        ...
        <div>
          <div className="bg-white border border-slate-100 rounded-3xl p-6 sm:p-8 shadow-md text-center mb-8">
            ...
            {/* Badge Details */}
            <div
              className={`border p-4 rounded-2xl max-w-sm mx-auto text-center ${gradeBadge.color}`}
            >
                  ...
                  >
                    ...
                    <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2 text-[11px] font-sans">
                      <div className="bg-slate-50 border border-slate-100/50 rounded-lg p-2.5">
                        <span className="block text-[9px] text-slate-400 font-mono uppercase font-bold">
                          Your Selection
                        </span>
                        <span
                          className={`block font-medium mt-0.5 ${isCorrect ? "text-emerald-800" : "text-slate-800"}`}
                        >
                          {q.options[userChoice] || "Bypassed"}
                        </span>
                      </div>
                      <div className="bg-emerald-500/3 border border-emerald-500/10 rounded-lg p-2.5">
                        <span className="block text-[9px] font-mono uppercase font-bold text-emerald-600">
                          Correct Choice
                        </span>
