...
import { GraduationCap, ArrowUpRight, Github, Info, BookOpen, User } from "lucide-react";
...
export default function App() {
  ...
  const handleGenerate = async (params: {
    ...
    try {
      ...
      if (!response.ok || !data.success) {
        throw new Error(data.error || "Generation query failed on the server. Please verify your file or search API constraints.");
      }
  ...
  return (
    <div className="min-h-screen bg-[#fafafa] flex flex-col font-sans text-slate-900 selection:bg-indigo-100 selection:text-indigo-900">
      ...
      <Header 
        topic={showAbout ? "About the Developer" : showAuth ? `${showAuth === 'login' ? "Access Account" : "Register with Cogito"}` : (topic || undefined)} 
        onBack={() => {
          if (showAbout) {
            setShowAbout(false);
          } else if (showAuth) {
        ...
        showBack={showAbout || !!showAuth || !!(flashcards || quiz || error || isLoading)} 
        onResetAll={topic && !showAbout && !showAuth ? handleReset : undefined}
        userEmail={userEmail}
        onLoginClick={() => { setShowAbout(false); setShowAuth('login'); }}
        onSignUpClick={() => { setShowAbout(false); setShowAuth('signup'); }}
        onLogout={async () => {
          if (isSupabaseConfigured && supabase) {
          ...
          <div className="max-w-md mx-auto py-12 px-4 text-center">
            ...
            {error.includes("Settings > Secrets") && (
              <div className="mt-4 bg-slate-50 border border-slate-100 rounded-xl p-3 text-[11px] text-slate-600 leading-normal font-sans text-left">
                <span className="font-semibold block text-slate-900 mb-0.5">Where is the API Key?</span>
                Look for the **Settings** menu at the top right of your workspace window, find the **Secrets** or **Environment Variables** panel, and configure <code className="font-mono bg-white px-1 border rounded text-[10px]">GEMINI_API_KEY</code> to enable secure live generations!
              </div>
            )}
        ...
        <div className="max-w-6xl mx-auto px-4 flex flex-col sm:flex-row items-center justify-between gap-3">
          <div className="flex flex-wrap items-center justify-center sm:justify-start gap-4">
            ...
            <div className="flex items-center space-x-3 text-slate-500">
              <button 
                onClick={() => { setShowAbout(false); setShowAuth(null); }}
              ...
              <span>•</span>
              <button 
                onClick={() => { setShowAbout(true); setShowAuth(null); }}
              ...
              <span>•</span>
              {userEmail ? (
                <span className="text-slate-400 font-mono text-[10px]">Logged In</span>
              ) : (
                <button 
                  onClick={() => { setShowAbout(false); setShowAuth('login'); }}
